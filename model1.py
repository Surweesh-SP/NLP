...
# ── zstd compression ──────────────────────────────────────────────────────────
# ── zstd compression (FAST VERSION) ────────────────────────────────
try:
    import zstandard as zstd
    def compress_bytes(b: bytes) -> bytes:
        return zstd.ZstdCompressor(level=3).compress(b)   # ⚡ changed from 22 → 3
    COMPRESSOR = "zstd-3"
except ImportError:
    def compress_bytes(b: bytes) -> bytes:
        return zlib.compress(b, level=6)  # faster fallback
    COMPRESSOR = "zlib-6"
...
class HP:
    ...
    num_recur_steps = int(os.environ.get("NUM_RECUR_STEPS",  "14"))     # 14 passes
    ...
    # Training
    seq_len         = int(os.environ.get("TRAIN_SEQ_LEN",     "512"))
    ...
    enable_checkpoint = bool(int(os.environ.get("ENABLE_CKPT", "1")))
# ==============================================================================
...
def main():
    ...
    def save_latest(step, best_val_bpb):
        async_save(_ckpt_state(step, best_val_bpb), ckpt_dir / "latest.pt")
    ...
    try:
        for step in range(start_step, hp.iterations + 1):
            ...
            # Periodic checkpoint
            # Periodic checkpoint (NON-BLOCKING + TOGGLE)
            if hp.enable_checkpoint and step % hp.ckpt_every == 0:
